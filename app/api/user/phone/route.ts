...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!updated || updated.length === 0) {
      console.error(
        "[PhoneUpdate] No rows updated! User might not exist in user_profiles."
      );
    ...
    console.log(
      "[PhoneUpdate] Phone updated successfully! Saved:",
      savedPhone,
      "User ID:",
      updated[0].user_id
    );
